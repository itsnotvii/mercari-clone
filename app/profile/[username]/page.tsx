      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6 flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Member since {new Date(profile.created_at).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
            <p className="text-sm text-gray-500 mt-1">{listings?.length || 0} active listing{listings?.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
              ...
              <Link href={`/listings/${listing.id}`} key={listing.id}>
                <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:-translate-y-1 transition-all duration-300 border border-gray-100">
                  ...
                  <div className="p-3">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-md font-medium mt-2 inline-block bg-gray-100 text-gray-400">{listing.condition}</span>
                  </div>
                </div>
              </Link>
            ))}
